...
import { TEntity } from '../entities/TEntity.ts';
 ...
 * 16文字の英数字、`-`および`_`から成る。
 */
export type TId<E extends TEntity> = TNominalPrimitive<string, 'TId'> & { __idBrand: E['__brand'] };
 ...
 */
export const isId = <E extends TEntity>(value: string): value is TId<E> => {
 ...
 * @returns 生成されたID文字列。
 */
export const generateId = <E extends TEntity>(): TId<E> => {
  ...
  const string = btoa(String.fromCharCode(...array))
    ...
    .replace(/\//g, '_');
  if (!isId(string)) {
    throw new NominalPrimitiveGenerationFailedError('IDを生成できません。');
  }
  return string;
};
