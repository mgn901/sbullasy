 ...
 * メールアドレスを表す公称プリミティブ型。
 */
export type TEmail = TNominalPrimitive<string, 'TEmail'>;

/**
 * 文字列がメールアドレスであるかを判定する関数。
 *
 * 文字列がHTML仕様の「valid e-mail address」に準拠するかを判定する。
 * 「valid email-address」はRFC 5322におけるメールアドレスの定義とは異なる。
 *
 * 参照: https://html.spec.whatwg.org/multipage/input.html#valid-e-mail-address
 *
 * @param value 判定対象の文字列。
 * @returns `value`がメールアドレスであれば`true`を返し、`value`を`TEmail`とみなす。そうでなければ`false`を返す。
 */
